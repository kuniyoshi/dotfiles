function fish_prompt
    # JOBS (suspendしているジョブの数)
    set -l job_count (count (jobs -c))
    if test $job_count -gt 0
        echo -n -s (set_color yellow) "(🎫$job_count)" (set_color normal)
    end

    # STASHES (git stashがある場合)
    ...
    # BRANCH (gitブランチ)
    if git rev-parse --is-inside-work-tree > /dev/null 2>&1
        set -l git_branch (git symbolic-ref HEAD | string replace "refs/heads/" "")
        echo -n -s " git(" (set_color blue) "$git_branch" (set_color normal) ")"
    end

    # CURRENT_PATH (現在のディレクトリ)
    echo -n -s " " (set_color green) (prompt_pwd) (set_color normal)

    # CURRENT_TIME (プロンプトの時間)
    echo -n -s " [" (set_color cyan) (date "+%y-%m-%d %H:%M:%S") (set_color normal) "]"
